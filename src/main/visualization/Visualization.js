  ...
  useEffect(() => {
    ...
    const svg = wrapper
      ...
      function mouseoverHandler() {
        ...
        if (data === null) {
          ...
          d3.select(".wrapper-2")
            .append("span")
            .text((d3.format(",")(data.cases)) + "  +" + (d3.format(",")(data.todayCases)));
          ...
          d3.select(".wrapper-3")
            .append("span")
            .text((d3.format(",")(data.deaths)) + "  +" + (d3.format(",")(data.todayDeaths)) );
          ...
          d3.select(".wrapper-4")
            .append("span")
            .text((d3.format(",")(data.recovered)) + "  +" + (d3.format(",")(data.todayRecovered)));
          toolTip.style("opacity", "1");
          toolTip.style("top", d3.event.pageY + "px");
          toolTip.style("left", d3.event.pageX - 200 + "px");
        }
    ...
    data.forEach((country, index) => {
        if(!index){
            ...
            highestCases = country.cases > highestCases? country.cases: highestCases;
            lowestCases = country.cases < lowestCases? country.cases: lowestCases;
        }
    });
    colorScale.domain([lowestCases, highestCases]).interpolator(d3.interpolatePiYG);
    const paths = svg
      .selectAll("path")
